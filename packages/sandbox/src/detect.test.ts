...
describe('generateDevcontainerConfig', () => {
  it('should generate config and Dockerfile for node stack', () => {
    ...
    expect(config.features).toEqual({ './security': {} })
    expect(config.workspaceMount).toBe(
      `source=${tempDir},target=/workspaces/${basename(tempDir)},type=bind`,
    )
    expect(config.workspaceFolder).toBe(`/workspaces/${basename(tempDir)}`)
  ...
  it('should skip agent install in Dockerfile when agent is none', () => {
    const { dockerfile } = generateDevcontainerConfig({
      workspacePath: tempDir,
      stack: 'node',
      sandboxId: 'test-none',
...
describe('hasExistingDevcontainer', () => {
  it('should return true when devcontainer.json exists', async () => {
    ...
    await writeFile(join(tempDir, '.devcontainer', 'devcontainer.json'), '{}')
  ...
  it('should return false when .devcontainer exists but no json file', async () => {
    await mkdir(join(tempDir, '.devcontainer'), { recursive: true })
    await writeFile(join(tempDir, '.devcontainer', 'Dockerfile'), 'FROM node:22')
