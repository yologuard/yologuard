...
describe('sandbox network isolation (E2E)', () => {
	...
	it('should not have bridge network access', async () => {
		...
		const { stdout } = await dockerExec({
			...
			command: 'docker inspect --format "{{json .NetworkSettings.Networks}}" ' + containerId! + ' 2>/dev/null || echo "skip"',
		})
	...
	it('should allow raw.githubusercontent.com with node-web egress preset', async () => {
		...
		const result = await dockerExec({
			containerId: containerId!,
			command: 'curl -sf --connect-timeout 10 https://raw.githubusercontent.com/yologuard/yologuard/refs/heads/main/README.md 2>&1',
			timeout: 20_000,
		})

		expect(result.exitCode).toBe(0)
		expect(result.stdout.toLowerCase()).toContain('yologuard')
	}, 30_000)
