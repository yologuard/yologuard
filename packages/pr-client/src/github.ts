...
export const createGitHubClient = ({
  ...
  const request = async (method: string, path: string, body?: unknown): Promise<unknown> => {
    ...
    if (!response.ok) {
      ...
      logger.error({ status: response.status, body: text, url }, 'GitHub API error')
  ...
  const createPR = async (params: GitHubPRParams): Promise<GitHubPR> => {
    const data = (await request('POST', `/repos/${params.owner}/${params.repo}/pulls`, {
      title: params.title,
      body: params.body ?? '',
      head: params.head,
      base: params.base,
  ...
  const getPR = async ({
    ...
    const data = (await request('GET', `/repos/${owner}/${repo}/pulls/${number}`)) as Record<
      string,
      unknown
    >
    return mapPR(data)
  ...
  const listPRs = async ({
    ...
    const data = (await request('GET', `/repos/${owner}/${repo}/pulls?${params}`)) as Record<
      string,
      unknown
    >[]
  ...
  const addLabels = async ({
    ...
    await request('POST', `/repos/${owner}/${repo}/issues/${number}/labels`, { labels })
  }
  ...
  const addComment = async ({
    ...
    const data = (await request('POST', `/repos/${owner}/${repo}/issues/${number}/comments`, {
      body,
    })) as Record<string, unknown>
