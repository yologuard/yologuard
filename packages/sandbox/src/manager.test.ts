...
const createFailingSpawn = (error: Error) => vi.fn().mockRejectedValue(error)
...
describe('createSandboxManager', () => {
  ...
  describe('destroySandbox', () => {
    ...
    it('should throw when devcontainer down fails', async () => {
      const logger = createMockLogger()
      const execFileImpl = vi.fn().mockRejectedValue(new Error('container not found'))
  ...
  describe('execInSandbox', () => {
    it('should run devcontainer exec and return output', async () => {
      ...
      expect(execFileImpl).toHaveBeenCalledWith(
        process.execPath,
        [DEVCONTAINER_JS, 'exec', '--workspace-folder', '/tmp/workspace', 'echo', 'hello world'],
        expect.objectContaining({ timeout: 300_000 }),
      )
    })
    ...
    it('should throw on unexpected errors', async () => {
      const logger = createMockLogger()
      const execFileImpl = vi.fn().mockRejectedValue(new Error('timeout exceeded'))
  ...
  describe('getSandboxStatus', () => {
    ...
    it('should return stopped when docker CLI inspect fails', async () => {
      const logger = createMockLogger()
      const execFileImpl = vi.fn().mockRejectedValue(new Error('no such container'))
