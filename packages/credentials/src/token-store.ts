...
export const createTokenStore = ({ logger }: TokenStoreParams): TokenStore => {
  ...
  const issueCredential = ({
    ...
    if (branch && !isAllowedBranch(branch)) {
      logger.warn({ sandboxId, branch }, 'Credential denied: protected branch')
      return undefined
    }

    // Check remote allowlist
    if (!isAllowedRemote({ sandboxId, remote })) {
      logger.warn({ sandboxId, remote }, 'Credential denied: unapproved remote')
      return undefined
    }
    ...
    logger.info({ sandboxId, remote, branch, expiresAt }, 'Issuing scoped credential')
