...
export const createGateway = async ({
  ...
  const sandboxManager = enableSandboxManager ? createSandboxManager({ logger }) : undefined

  const dockerClient = enableSandboxManager ? createDockerClient({ logger }) : undefined
  ...
  const destroySandboxById = async (sandboxId: string) => {
    ...
    // For generated configs, devcontainer up used the config base dir as --workspace-folder.
    // configPath = ~/.yologuard/configs/<id>/.devcontainer/devcontainer.json
    // configBaseDir = ~/.yologuard/configs/<id>
    const workspacePath = sandbox.configPath
      ? dirname(dirname(sandbox.configPath))
      : sandbox.repo

    if (sandboxManager && workspacePath) {
      try {
    ...
    // configPath lives under ~/.yologuard/configs/<id>/ — remove the whole config base
    if (sandbox.configPath) {
      try {
        await rm(dirname(dirname(sandbox.configPath)), { recursive: true, force: true })
      } catch {
  ...
  const deps: RouteDeps = {
    ...
    inspectContainer,
    resolveDevcontainerConfig,
    copySecurityFeature: enableSandboxManager ? copySecurityFeature : undefined,
    launchAgent,
    startHealthMonitor,
    stopHealthMonitor,
    userConfig,
    ...
    try {
      ...
      for (const container of containers) {
        ...
        if (!knownContainerIds.has(shortId) && !knownContainerIds.has(container.Id)) {
          logger.info(
            { containerId: container.Id, names: container.Names },
