...
const mockResolveConfig = vi.fn(async ({ sandboxId }: { sandboxId: string }) => ({
	config: {
		...
		runArgs: ['--network', 'none', '--label', 'yologuard=true', '--label', `yologuard.sandbox-id=${sandboxId}`],
		remoteUser: 'node',
...
const createDestroyFn = ({ store, egress, sandboxManager }: {
	...
	if (sandboxManager) {
		try { await sandboxManager.destroySandbox({ id: sandboxId, workspacePath: sandbox.repo, logger }) } catch { /* ignore */ }
	}
	if (egress) {
		try { await egress.destroySidecar({ sandboxId, logger }) } catch { /* ignore */ }
		try { await egress.destroySandboxNetwork({ sandboxId, logger }) } catch { /* ignore */ }
	}
	store.remove(sandboxId)
}

const setupTest = async ({ egress, store, sandboxManager, resolveDevcontainerConfig }: {
	readonly egress?: EgressDeps
	readonly store: SandboxStore
	readonly sandboxManager?: ReturnType<typeof createMockSandboxManager>
