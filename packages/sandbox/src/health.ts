		...
		interval: setInterval(async () => {
			try {
				...
				if (!info.State.Running) {
					logger.warn(
						{ sandboxId, status: info.State.Status },
				...
				if (monitor) {
					...
					if (idleDuration >= monitor.idleTimeoutMs) {
						logger.info(
							{ sandboxId, idleDuration },
							'Sandbox idle timeout reached',
						)
...
export const isMonitoring = (sandboxId: string): boolean =>
	monitors.has(sandboxId)
