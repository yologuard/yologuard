...
export const setup = async () => {
  ...
  try {
    ...
    const portStr = await rl.question(`Gateway port [${currentPort ?? DEFAULT_GATEWAY_PORT}]: `)
    ...
    const agent = await rl.question(`Default agent [${currentAgent ?? 'claude'}]: `)
    ...
    const policy = await rl.question(`Default network policy [${currentPolicy ?? 'none'}]: `)

    // GitHub PAT
    const pat = await rl.question('GitHub personal access token (optional, leave blank to skip): ')
