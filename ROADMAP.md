...
- [x] WebSocket endpoint for real-time events (`/ws`)
...
- [x] Unix socket listener for in-sandbox communication (`/yologuard/gateway.sock`)
...
- [x] `SandboxManager` module — wraps `@devcontainers/cli` (`devcontainer up`, `devcontainer exec`, `devcontainer down`)
- [x] Auto-detect repo stack (Node, Python, Go, Rust) and generate `devcontainer.json` when none exists
- [x] Support existing `.devcontainer/devcontainer.json` seamlessly
- [x] Mount workspace directory into container
- [x] tmux session inside container for persistent agent terminal
- [x] Agent launcher — start Claude Code / Codex / opencode inside tmux with `--dangerously-skip-permissions` (inside sandbox, this is safe)
- [x] Container health monitoring (heartbeat check, OOM detection)
- [x] Deterministic cleanup — destroy container + prune worktrees on stop
- [x] Idle timeout — auto-destroy after configurable period (default: 30 min)
- [x] Resource limits: CPU, memory, disk quota per container
...
- [x] Launch containers with `--network=none` (no network interfaces except loopback)
- [x] Create dedicated Docker internal network per sandbox for sidecar communication
- [x] Squid sidecar container per sandbox:
...
- [x] Credential helper denies auth for:
  - Unapproved remotes
...
- [x] `pr-client` package: minimal REST client for GitHub API
  ...
  - Add labels, add comments
  - Git remote URL parser (HTTPS, SSH, shortform)
...
- [x] GitHub.com support only in v0.1 (GitLab in v0.2)
...
- [x] `audit` package: SQLite via `better-sqlite3`
- [x] Per-sandbox SQLite DB at `~/.yologuard/audit/<sandbox-id>.db`
- [x] Log entries:
...
- [x] Size cap: default 50MB per DB, oldest-first pruning (approval records never pruned)
- [x] Secret redaction pass: regex-based, best-effort (`Bearer`, `ghp_`, `sk-`, `AKIA`, high-entropy strings)
- [x] `yologuard audit <sandbox-id>` CLI command — query and display audit log
- [x] Structured JSON output option for programmatic access (`--json`)
...
- [x] Host-side bare clone cache at `~/.yologuard/repos/`
  - `git clone --bare` on first setup
  - `git fetch` for incremental updates
- [x] Git worktrees from bare clones mounted into sandbox workspace
  - `git worktree add /sandbox/workspace/<repo-name> HEAD`
  - Near-instant, no object duplication
- [x] Per-repo access control: `read-write` vs `readonly` (readonly = no credential helper creds for push)
- [x] Sparse checkout support for large repos (configurable per repo)
- [x] `yologuard warm` command — pre-fetch all repos for CI warm cache
- [x] Workspace config: `repos[]` array with URL, access level, sparse paths
- [x] Worktree cleanup on sandbox destroy (`git worktree prune`)
...
- [x] `yologuard attach <sandbox>` — connect to agent's tmux session
- [x] `yologuard stop <sandbox>` / `yologuard destroy <sandbox>` — cleanup
- [x] `yologuard logs <sandbox>` — stream sandbox logs
...
- [x] `yologuard warm` — pre-fetch repos
...
- [x] Base sandbox Dockerfile (`images/sandbox/Dockerfile`):
