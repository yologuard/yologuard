...
export const registerModelProxy = ({ app, logger, getApiKey }: RegisterModelProxyParams) => {
  ...
  app.all('/v1/*', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!provider) {
      return reply.status(400).send({
        status: 400,
        error:
          'Unknown model API path. Supported: /v1/messages (Anthropic), /v1/chat/completions (OpenAI)',
      })
    }
    ...
    try {
      ...
      if (provider === 'anthropic') {
        ...
        headers['anthropic-version'] =
          (request.headers['anthropic-version'] as string) ?? '2023-06-01'
      } else {
